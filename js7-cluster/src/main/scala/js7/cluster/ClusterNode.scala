...
object ClusterNode:
  ...
  : Resource[IO, ClusterNode[S]] =
    ...
    def prepareBackupNodeWithEmptyClusterState(): Prepared[S] =
      ...
      val passiveClusterNode: IO[PassiveClusterNode[S]] = IO.fromFuture(IO.pure(startedPromise.future))
        .map(cmd =>
    ...
    def startAsActiveNodeWithBackup(): Prepared[S] =
      recovered.clusterState match
        case recoveredClusterState: Coupled =>
          ...
          val failedOver: IO[Option[(Recovered[S], PassiveClusterNode[S])]] = common

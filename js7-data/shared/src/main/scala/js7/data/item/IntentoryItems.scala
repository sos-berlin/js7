...
object IntentoryItems
{
  ...
  private def toAddedOrChanged(previousPathToItem: TypedPath => Option[InventoryItem], ignoreVersion: Boolean)(item: InventoryItem): Option[RepoChange] =
  ...
  final case class Diff[P <: TypedPath, A <: InventoryItem](added: Seq[A] = Nil, updated: Seq[A] = Nil, deleted: Seq[P] = Nil)
  {
    ...
    /** Returns a subset of a certain `TypedPath` and `InventoryItem`. */

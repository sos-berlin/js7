...
private[js7] object TestSetting
{
  ...
  val AJob = WorkflowJob(TestAgentPath, APathExecutable, Map("JOB_A" -> StringValue("A-VALUE")), parallelism = 3)
  val BJob = WorkflowJob(TestAgentPath, BPathExecutable, Map("JOB_B" -> StringValue("B-VALUE")), parallelism = 3)
  val B1Job = WorkflowJob(TestAgentPath, BPathExecutable, Map("JOB_B1" -> StringValue("B1-VALUE")), parallelism = 3)

...
final class PositionOrLabelTest extends OurTestSuite:
  ...
  "Position" - {
    ...
    "nextRetryBranchPath" in:
      assert(Position(0).nextRetryBranchPath == Left(Problem("Not in a try or catch block")))
      assert(Position(99).nextRetryBranchPath == Left(Problem("Not in a try or catch block")))
      assert((Position(1) / try_(0) % 0).nextRetryBranchPath == Left(Problem("Not in a try or catch block")))
      assert((Position(1) / try_(1) % 0).nextRetryBranchPath == Left(Problem("Not in a try or catch block")))

...
final class JLockStateTest extends OurTestSuite:
  ...
  private val exclusiveLockState = JLockState(LockState(lock, Exclusive(OrderId("A")), Vector(OrderId("Q"))))
  private val nonExclusiveLockState = JLockState(LockState(lock, NonExclusive(Map(OrderId("A") -> 1)), Vector(OrderId("Q"))))

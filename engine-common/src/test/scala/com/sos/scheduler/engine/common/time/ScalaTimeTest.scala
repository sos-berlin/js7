...
final class ScalaTimeTest extends FreeSpec {
  ...
  "Duration" - {
    ...
    "Int.hours" in {
      (7.hours: Duration).toHours shouldEqual 7
      (7.hours: Duration).toMillis shouldEqual (7*3600*1000)
    }
    "Long.hours" in {
      (7L.hours: Duration).toHours shouldEqual 7
      (7L.hours: Duration).toMillis shouldEqual (7*3600*1000)
    }
    "Int.days" in {

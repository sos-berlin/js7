...
final class ClusterWatchTest extends OurTestSuite
{
  ...
  private var clusterState: ClusterState = ClusterState.NodesAppointed(setting)
  private lazy val watch = new ClusterWatch(ControllerId("CONTROLLER"), () => MonixDeadline.now(scheduler))
  ...
  private def applyEvents(from: NodeId, events: Seq[ClusterEvent]): Checked[Completed] = {

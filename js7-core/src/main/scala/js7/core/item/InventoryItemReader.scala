...
import js7.data.item.{InventoryItem, ItemId, ItemId_, SourceType, TypedPath}
...
trait InventoryItemReader
{
  ...
  protected def read(id: ItemId[ThisTypedPath], byteArray: ByteArray): PartialFunction[SourceType, Checked[ThisItem]]
  ...
  private[item] def readUntyped(id: ItemId_, byteArray: ByteArray, sourceType: SourceType): Checked[ThisItem] = {
    ...
    val result: Checked[ThisItem] = read(id.asInstanceOf[ItemId[ThisTypedPath]], byteArray).applyOrElse(sourceType,

...
import org.apache.pekko.http.scaladsl.model.StatusCodes.{Forbidden, NotFound}
...
trait LogRoute extends RouteProvider:
  ...
  final lazy val logRoute: Route =
    authorized(ValidUserPermission):
      path(Segment):
        case "error" => fileRoute(LogLevel.Error)
        case "info" => fileRoute(LogLevel.Info)
        case "debug" => fileRoute(LogLevel.Debug)
        case "none" =>
        ...
        case o => complete(NotFound, s"Unknown logLevel: $o")
      ~
        pathEnd:
          fileRoute(LogLevel.Info) // Convenience for testing with Curl
  ...
  private def fileRoute(logLevel: LogLevel): Route =
    seal:
      ...
      (
        ...
        val growing = maybeGrowing.getOrElse:
          !beginString.isDefined // Inofficial comfort for testing with curl
          ...
          completeWithStream(`text/plain(UTF-8)`):
            ...
            .through: stream =>
              if withKey then
                ...
                .map(_.toByteString)
            // We send heartbeats because
            // - recompressing and indexing may take some time
            // - pattern parameter may search a long time
            .keepAlive(
...
object LogRoute:
  ...
  private val LogHeartbeat = IO.pure(ByteString(LogReaders.LogHeartbeat))

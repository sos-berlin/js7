...
object JavaMainLockfileSupport:
  ...
  def runMain[R](name: String, args: Seq[String], useLockFile: Boolean = false)
    (body: CommandLineArguments => IO[R])
  : IO[R] =
    if useLockFile then
  ...
  private def lockAndRunMain[R](name: String, args: Seq[String])
    (body: CommandLineArguments => IO[R])
  : IO[R] =
    IO.defer:
      val arguments = CommandLineArguments(args)

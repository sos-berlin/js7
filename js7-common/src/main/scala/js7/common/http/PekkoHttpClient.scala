...
import org.apache.pekko.http.scaladsl.model.{ContentType, ContentTypes, ErrorInfo, HttpEntity, HttpHeader, HttpMethod, HttpRequest, HttpResponse, RequestEntity, StatusCode, Uri as PekkoUri}
...
trait PekkoHttpClient extends AutoCloseable with HttpClient with HasIsIgnorableStackTrace
{
  ...
  private def endStreamOnNoMoreElementNeeded: PartialFunction[Throwable, Observable[Nothing]] = {
    case t @ org.apache.pekko.stream.SubscriptionWithCancelException.NoMoreElementsNeeded =>
  ...
  private def ignoreIdleTimeout: PartialFunction[Throwable, Observable[Nothing]] = {
    case t: org.apache.pekko.stream.scaladsl.TcpIdleTimeoutException =>
  ...
  : Task[B] =   {
    ...
    Task.defer {
      ...
      chunks
        ...
        .flatMap(pekkoChunks =>
          sendReceive(
            HttpRequest(POST, uri.asPekko, Accept(`application/json`) :: Nil,
    ...
    withCheckedAgentUri(request)(request =>
      sessionTokenTask.flatMap { sessionToken =>
        ...
        Task
          ...
          .guaranteeCase {
            ...
            case ExitCase.Error(throwable) => Task.defer {
              val mark = throwable match {
                case t: org.apache.pekko.stream.StreamTcpException if t.getMessage.contains(
          ...
          .onErrorRecoverWith {
            case t: org.apache.pekko.stream.StreamTcpException => Task.raiseError(makePekkoExceptionLegible(t))
          }
  ...
  override def isIgnorableStackTrace(throwable: Throwable) = throwable match {
    case _: org.apache.pekko.stream.StreamTcpException => false
    case _ => true
  }

  private def makePekkoExceptionLegible(t: org.apache.pekko.stream.StreamTcpException): RuntimeException =
...
object PekkoHttpClient
{
  ...
  def toPrettyProblem(throwable: Throwable): Problem = {
    ...
    throwable match {
      case org.apache.pekko.http.scaladsl.model.EntityStreamException(ErrorInfo(summary, _)) =>

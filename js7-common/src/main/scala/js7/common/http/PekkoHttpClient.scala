          ...
          .guaranteeCase {
            ...
            case ExitCase.Error(throwable) => Task.defer {
              val sym = throwable match {
                ...
                case _: org.apache.pekko.stream.scaladsl.TcpIdleTimeoutException => "🔥"
...
object PekkoHttpClient
{
  ...
  private val pekkoExceptionRegex = new Regex("pekko.stream.StreamTcpException: Tcp command " +
  ...
  private final class LegiblePekkoHttpException(
    message: String,
    cause: org.apache.pekko.stream.StreamTcpException)

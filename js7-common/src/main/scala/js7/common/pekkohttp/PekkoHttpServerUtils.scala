...
object PekkoHttpServerUtils
{
  ...
  private def eatParameterOption(parameterMap: ParameterMap, key: String) =
    new Directive[ParameterMap :: Option[String] :: ())] {
      override def tapply(inner: (ParameterMap :: Option[String] :: ())) => Route) =
        inner((parameterMap - key) :: parameterMap.get(key) :: ()))
    }

  private def eatParameter(parameterMap: ParameterMap, key: String) =
    new Directive[ParameterMap :: String :: ())] {
      override def tapply(inner: (ParameterMap :: String :: ())) => Route) =
        parameterMap.get(key) match {
          case Some(value) => inner((parameterMap - key) :: value :: ()))
          case None => reject
        }
    }

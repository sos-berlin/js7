...
trait SessionRoute extends RouteProvider:
          ...
          preAuthenticate: idsOrUser =>
            entity(as[SessionCommand]): command =>
              requestSource: source =>
                onSuccess(execute(command, source, idsOrUser, tokenOption).unsafeToFuture()):
  ...
  private def execute(
    ...
    sessionTokenOption: Option[SessionToken])
  : IO[Checked[SessionCommand.Response]] =
    command match
      case Login(userAndPasswordOption, version) =>
        IO(authenticateOrUseHttpUser(idsOrUser, userAndPasswordOption)).flatMapT: user =>
          sessionRegister.login(user, source, version, sessionTokenOption)
  ...
  : Checked[SimpleUser] =
    userAndPasswordOption match
      case None =>
        idsOrUser match
          case Left(_) => Left(specificLoginRequiredProblem)
          case Right(u) => Right(u)

      case Some(userAndPassword) =>
        if userAndPassword.userId.isAnonymous then

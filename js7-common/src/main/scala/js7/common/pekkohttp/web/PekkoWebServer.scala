 ...
 */
trait PekkoWebServer extends AutoCloseable
{
  ...
  private lazy val pekkoHttp = Http(actorSystem)
...
object PekkoWebServer
{
  ...
  @TestOnly
  def forTest()(route: Route)(implicit as: ActorSystem): PekkoWebServer & HasUri =
    forTest(ConfigFactory.empty)(route)

  @TestOnly
  def forTest(config: Config)(route: Route)(implicit as: ActorSystem): PekkoWebServer & HasUri =
  ...
  extends PekkoWebServer with HasUri
  {
  ...
  : Resource[Task, PekkoWebServer & HasUri] =
    resource(

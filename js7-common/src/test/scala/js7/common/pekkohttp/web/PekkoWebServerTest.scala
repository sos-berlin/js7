...
import js7.common.pekkohttp.web.PekkoWebServer.HasUri
...
final class PekkoWebServerTest extends OurTestSuite with BeforeAndAfterAll
{
  ...
  private lazy val webServer = new PekkoWebServer with HasUri {
    ...
    protected def actorSystem = PekkoWebServerTest.this.actorSystem
    ...
    def newBoundRoute(binding: WebServerBinding, whenTerminating: Future[Deadline]) =
      Task(PekkoWebServer.BoundRoute(
        path("TEST") {
          complete("OKAY")
        },
        whenTerminating))

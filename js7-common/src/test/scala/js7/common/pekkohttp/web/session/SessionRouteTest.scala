...
extends OurTestSuite, SessionRouteTester:
  ...
  private def withSessionApi(
    ...
    (body: HttpSessionApi & PekkoHttpClient & SessionApi.HasUserAndPassword => Unit)
  : Unit =
    val api = new HttpSessionApi with PekkoHttpClient with SessionApi.HasUserAndPassword:
      protected val name = "SessionRouteTest"
      protected def userAndPassword = userAndPassword_
      def httpClient = this: PekkoHttpClient

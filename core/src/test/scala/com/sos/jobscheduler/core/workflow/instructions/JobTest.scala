...
import com.sos.jobscheduler.data.workflow.{OrderContext, Position, WorkflowPath, WorkflowPosition}
...
final class JobTest extends FreeSpec {
  ...
  private def toEvent(outcome: Outcome): Option[KeyedEvent[OrderActorEvent]] = {
    val order = Order(orderId, (WorkflowPath("/WORKFLOW") % "VERSION" ) /: Position(1, 2, 3), Order.Processed(outcome))
    JobExecutor.toEvent(orderContext, order, job)
  }
}

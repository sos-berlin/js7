...
import com.sos.jobscheduler.core.common.jsonseq.InputStreamJsonSeqIteratorTest._
...
/**
  ...
  */
final class InputStreamJsonSeqIteratorTest extends FreeSpec {

  "InputStreamJsonSeqIterator" in {
    for (blockSize ← 1 to 2 * FourByteUtf8.length; charBufferSize ← 1 to FourByteUtf8.length) {
      for (n ← 0 to 3) {
        withClue(s"blockSize=$blockSize charBufferSize=$charBufferSize n=$n:\n") {
          ...
          val posAndJsons = new InputStreamJsonSeqIterator(in, blockSize = blockSize, charBufferSize = charBufferSize).toVector
...
private object InputStreamJsonSeqIteratorTest {

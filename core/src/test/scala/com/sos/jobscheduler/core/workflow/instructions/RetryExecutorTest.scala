...
final class RetryExecutorTest extends FreeSpec
{
  "toEvent" in {
    ...
    assert(toEvent(tryPosition / "try+0" % 0) == Invalid(Problem("Retry, but not in a catch-block")))
  }

  "Find try Position and increment BranchId" in {
    assert(toEvent(tryPosition / "catch+0" % 0) == Valid(Some(orderId <-: OrderRetrying(tryPosition / "try+1" % 0))))
  ...
  "Delay" in {
    ...
    assert(toEvent(tryPosition / "catch+0" % 0, delays) == Valid(Some(orderId <-: OrderRetrying(tryPosition / "try+1" % 0, None))))

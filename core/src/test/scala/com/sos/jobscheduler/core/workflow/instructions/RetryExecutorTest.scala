...
{
  "toEvent" in {
    assert(toEvent(Position(1)) == None)  // error
    assert(toEvent(tryPosition) == None)  // error
    assert(toEvent(tryPosition / "try" % 0) == None)  // error
  }

  "Find try Position and increment BranchId" in {
    assert(toEvent(tryPosition / "catch"   % 0) == Some(orderId <-: OrderRetrying(tryPosition / "try+1" % 0)))
    assert(toEvent(tryPosition / "catch+1" % 0) == Some(orderId <-: OrderRetrying(tryPosition / "try+2" % 0)))
  }

  "Delay" in {
    val delays = 0.second :: 1.second :: 2.seconds :: 11.5.seconds :: Nil
    assert(toEvent(tryPosition / "catch"   % 0, delays) == Some(orderId <-: OrderRetrying(tryPosition / "try+1" % 0, None)))
    assert(toEvent(tryPosition / "catch+1" % 0, delays) == Some(orderId <-: OrderRetrying(tryPosition / "try+2" % 0, Some(now + 1.second))))
    assert(toEvent(tryPosition / "catch+2" % 0, delays) == Some(orderId <-: OrderRetrying(tryPosition / "try+3" % 0, Some(now + 2.seconds))))
    assert(toEvent(tryPosition / "catch+3" % 0, delays) == Some(orderId <-: OrderRetrying(tryPosition / "try+4" % 0, Some(now + 12.seconds))))
    assert(toEvent(tryPosition / "catch+4" % 0, delays) == Some(orderId <-: OrderRetrying(tryPosition / "try+5" % 0, Some(now + 12.seconds))))
  }
}

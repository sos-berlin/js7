...
final class InstructionContextTest extends FreeSpec
{
  ...
  "toRetryCount" in {
    ...
    assert(context.toRetryCount(TestWorkflow.id /: (Position(1) / 1 % 0)) == 1)
    assert(context.toRetryCount(TestWorkflow.id /: (Position(1) / 2 % 0)) == 2)
  ...
  "toRetryPosition" in {
    ...
    assert(context.toRetryPosition(TestWorkflow.id /: (Position(1) / 1    % 0)) == Invalid(Problem("Not in a catch-block")))
    assert(context.toRetryPosition(TestWorkflow.id /: (Position(1) / Catch_ % 0)) == Valid(Position(1) / 1 % 0))
    assert(context.toRetryPosition(TestWorkflow.id /: (Position(1) / -1     % 0)) == Valid(Position(1) / 2 % 0))
    assert(context.toRetryPosition(TestWorkflow.id /: (Position(1) / -2     % 0)) == Valid(Position(1) / 3 % 0))
  }
}

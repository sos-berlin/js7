...
final class AgentUrisTest extends FreeSpec {
  ...
  "command" in {
    assert(agentUris.command ==
      Uri("http://example.com:9999/testPrefix/agent/api/command"))
  }

  "task" - {
    "overview" in {
      assert(agentUris.task.overview ==
        Uri("http://example.com:9999/testPrefix/agent/api/task"))
    }
  ...
  "task.tasks" in {
    assert(agentUris.task(AgentTaskId("TASK-ID")) ==
      Uri("http://example.com:9999/testPrefix/agent/api/task/TASK-ID"))
  }

  "overview" in {
    assert(agentUris.overview ==
      Uri("http://example.com:9999/testPrefix/agent/api"))
  }
}

...
object CirceJsonTester {
  ...
  def testJson[A: Encoder: Decoder](a: A, json: ⇒ Json): Unit = {
    ...
    if (json != asJson) fail(s"${prettyPrinter.pretty(normalize(json))} did not equal ${prettyPrinter.pretty(normalize(asJson))} ")
    assert(forceRight(json.as[A]) == a)
  ...
  private def parseJson(string: String): Json =
    forceRight(parse(string))

  private def forceRight[R](either: Either[Throwable, R]): R =
    either match {
      case Right(o) ⇒ o

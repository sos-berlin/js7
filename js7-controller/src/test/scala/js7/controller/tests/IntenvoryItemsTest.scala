...
import js7.controller.tests.IntenvoryItemsTest._
import js7.data.agent.{AgentRef, AgentRefPath}
import js7.data.item.IntenvoryItems.diffInventoryItems
import js7.data.item.{IntenvoryItems, InventoryItem, RepoChange, TypedPath, VersionId}
...
/**
  ...
  */
final class IntenvoryItemsTest extends AnyFreeSpec
{
  "diffInventoryItems" - {
    "empty"  in {
  ...
  "Diff" in {
    val diff = IntenvoryItems.Diff.fromRepoChanges(
      List(
    ...
    assert(diff == IntenvoryItems.Diff[TypedPath, InventoryItem](
    ...
    assert(diff.select[WorkflowPath, Workflow] ==
      IntenvoryItems.Diff[WorkflowPath, Workflow](
        added = List(CWorkflow withVersion V1),
        updated = List(D1Workflow withVersion V1),
        deleted = List(BWorkflow.path)))

    assert(diff.select[AgentRefPath, AgentRef] ==
      IntenvoryItems.Diff[AgentRefPath, AgentRef](

...
import js7.controller.data.{ControllerState, ControllerStateBuilder}
...
object ControllerJournalRecoverer
{
  def recover(journalMeta: JournalMeta, config: Config, runningSince: Deadline = now): Recovered[ControllerState] =
    JournaledStateRecoverer.recover(journalMeta, ControllerState.Undefined, () => new ControllerStateBuilder, config, runningSince)
}

...
trait GraphqlRoute extends ControllerRouteProvider
{
  ...
  final val graphqlRoute: Route =
    ...
    path("schema") {
      completeWith(StringMarshaller)(_(ControllerGraphqlSchema.schema.renderPretty))
    }
  ...
  private def executeGraphql(query: Document, operationName: Option[String], variables: Json): Future[Json] =
    Executor.execute(ControllerGraphqlSchema.schema, query, context,
...
object GraphqlRoute
{
  private val GraphiqlResource = JavaResource("js7/controller/web/controller/api/graphql/graphiql.html")
}

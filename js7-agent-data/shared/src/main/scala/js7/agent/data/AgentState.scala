...
extends SignedItemContainer,
  ...
  def toSnapshotStream = Stream(
    ...
    Stream.fromIterable(meta != AgentMetaState.empty thenList meta),
    Stream.fromIterable(keyToItem(AgentRef).values),
    ...
    Stream.fromIterable(keyTo(FileWatchState).values).flatMap(_.toSnapshotStream),
    Stream.fromIterable(keyToSignedItem.values.view.map(SignedItemAdded(_))),
    Stream.fromIterable(idToWorkflow.view.filterKeys(isWithoutSignature).values),
    Stream.fromIterable(pathToJobResource.view.filterKeys(isWithoutSignature).values),

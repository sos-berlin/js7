...
object AgentControllerEvent
{
  ...
  final case class AgentReadyForController(timezone: String, totalRunningTime: FiniteDuration) extends AgentControllerEvent

  implicit val jsonCodec = TypedJsonCodec[AgentControllerEvent](
    Subtype(deriveCodec[AgentReadyForController]))
}

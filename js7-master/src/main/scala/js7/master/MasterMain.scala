...
final class MasterMain
{
  ...
  def run(arguments: CommandLineArguments): MasterTermination.Terminate = {
    logger.info(s"JS7 JobScheduler Master ${BuildInfo.prettyVersion}")  // Log early for early timestamp and proper logger initialization by a single (not-parallel) call

...
import com.sos.jobscheduler.data.workflow.{JobPath, NodeId, Workflow, WorkflowPath, WorkflowGraph}
...
private[jobscheduler] object TestSetting {
  ...
  val TestWorkflow = Workflow(WorkflowPath("/WORKFLOW"), WorkflowGraph(start = A.id, nodes = List(A, B, END), transitions = List(a, b)))

...
object MasterMain {
  ...
  def main(args: Array[String]): Unit = {
    ...
    try
      ...
      println(s"JOBSCHEDULER MASTER TERMINATED DUE TO ERROR")
      Log4j.shutdown()
      sys.runtime.exit(1)
    }
  }
  ...
  def start(conf: MasterConfiguration): Future[RunningMaster] =
    for (master ← RunningMaster(conf)) yield {
      ...
      master.executeCommandAsSystemUser(MasterCommand.ScheduleOrdersEvery(OrderScheduleDuration.toFiniteDuration)).runAsync // Will block on recovery until Agents are started: await 99.s

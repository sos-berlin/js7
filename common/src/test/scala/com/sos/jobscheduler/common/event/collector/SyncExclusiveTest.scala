...
final class SyncExclusiveTest extends FreeSpec {
  ...
  "test" in {
    ...
    for ((aEventId, bEventId) <- List((1L, 2L), (3L, 4L), (5L, 6L))) {
      val a = sync.whenEventIsAvailable(aEventId, until = None).runToFuture
      assert(!a.isCompleted)
      val b = sync.whenEventIsAvailable(bEventId, until = None).runToFuture
      ...
      assert(!sync.whenEventIsAvailable(aEventId, until = None).runToFuture.isCompleted)
      assert(!sync.whenEventIsAvailable(aEventId, until = None).runToFuture.isCompleted)
    }
  }
  ...
  "timeout" in {
    ...
    for (eventId <- 1L to 3L) {
      withClue(s"#$eventId") {
        val a = sync.whenEventIsAvailable(eventId, until = Some(now + 2*tick), delay = 2*tick).runToFuture
        val b = sync.whenEventIsAvailable(eventId, until = Some(now + 1.hour), delay = 2*tick).runToFuture
        assert(a ne b)
  ...
  if (sys.props contains "test.speed") "speed" in {
    ...
    for (_ <- 1 to 10) {
      ...
      val futures: Seq[Future[Boolean]] =
        (for (eventId <- eventIds) yield Future { sync.whenEventIsAvailable(after = eventId - 1, Some(now + 99.seconds)).runToFuture })
          .map({ _.flatten })

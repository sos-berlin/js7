    ...
    def build(workflow: Workflow): Vector[(Position, Int, Int)] = {
      ...
      def renderNested(labeledInstructions: Seq[Instruction.Labeled], parents: Position.Parents, nesting: Int): Unit = {
        for ((labeled, nr) ← labeledInstructions.zipWithIndex) {
          ...
          positionsWithXY += ((position, nestingPx(nesting), lastY))
          lastY += InstructionHeight
          addVdom(<.div(^.cls := "orders-Instruction", moveElement(nestingPx(nesting), lastY), instructionToVdom(labeled)))
          labeled.instruction match {
            case ForkJoin(branches) ⇒
              for (branch ← branches) {
                addVdom(<.div(^.cls := "orders-Branch", moveElement(nestingPx(nesting + 0.4), lastY + BranchIdY), branch.id.string))
            ...
            case _if: If ⇒
              ...
              if (_if.elseWorkflow.isDefined) {
                addVdom(<.div(^.cls := "orders-Branch", moveElement(nestingPx(nesting), lastY + ElseY), "else"))
                renderBranch(1)  // else
              }

...
final class SimpleDirectoryWatcherTest extends AnyFreeSpec
{
  ...
  "step for step" in {
    withTemporaryDirectory("DirectoryWatcherTest-") { dir =>
      dir / "1" := ""
      val watcher = new SimpleDirectoryWatcher(
        WatchOptions(dir, Set(ENTRY_CREATE, ENTRY_DELETE, ENTRY_MODIFY)))

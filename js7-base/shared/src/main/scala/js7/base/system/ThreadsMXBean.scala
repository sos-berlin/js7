...
object ThreadsMXBean:
  ...
  object Bean extends ThreadsMXBean:
    ...
    private var _computed: Computed = Computed(Array.empty, 0)
    ...
    protected[ThreadsMXBean] def computed(): Computed =
      ...
      if elapsed >= CacheDuration then
        //meter:
          val activeCount = Thread.activeCount() // May differ from threads.length ?
          val threads = ManagementFactory.getThreadMXBean.dumpAllThreads(false, false, 0)
          _computed = Computed(threads, activeCount)
      _computed


  private final class Computed(threads: Array[ThreadInfo], activeCount: Int):
    val g1GCThreads =
    ...
    val otherThreads =
      (threads.length max activeCount)
        - g1GCThreads

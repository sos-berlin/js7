...
final class ToSuperclass[C](superclasses: Set[Class[_ <: C]])(implicit C: ClassTag[C])
{
  private val map = new ConcurrentHashMap[Class[_ <: C], Option[Class[_ <: C]]]
  map.putAll(superclasses.view.map(o => o -> Some(o)).toMap.asJava)

  def checked[C1 <: C](cls: Class[C1]): Checked[Class[_ <: C]] =
  ...
  def get[C1 <: C](cls: Class[C1]): Option[Class[_ <: C]] =
    map.get(cls) match {
      case null =>
        synchronized {
          map.get(cls) match {
            case null =>
              val maybeSuperclass = superclassesOf(cls)
                .view
                .flatMap(superclass => Option(map.get(superclass.asInstanceOf[Class[_ <: C]])))
                .headOption
                .flatten

...
final class Memoizer[A, B](f: A => B, show: Memoizer.Show[A, B]) extends (A => B):
  ...
  def toMap: Map[A, B] = memory.toMap

  override def toString: String = show.show(this)
...
object Memoizer:
  ...
  /**
    ...
    * For strictly one computation, see `Memoizer.strict`.
    */
  def nonStrict3[A, B, C, R](f: (A, B, C) => R)
    (using show: Show[(A, B, C), R] = defaultShow[(A, B, C), R])
  : (A, B, C) => R =
  ...
  /**
    ...
    */
  def strict1[A, B](f: A => B)(using show: Show[A, B] = defaultShow[A, B])
  : A => B =
  ...
  /**
    * Memoizes all computed results and computes each result strictly once.
    */
  def strict2[A, B, R](f: (A, B) => R)(using show: Show[(A, B), R] = defaultShow[(A, B), R])
  : (A, B) => R =

...
object WaitForCondition:
  ...
  private[time] trait Sleeper:
    def now: Deadline
    final def sleepUntil(until: Deadline) = sleep(until - now)
  ...
  def retryUntil[A](timeout: FiniteDuration, step: FiniteDuration)(body: => A)
    (using sleeper: Sleeper = defaultSleeper)
  : A =
  ...
  /** Wartet bis zu den relative Zeitpunkten, bis condition wahr wird.
    ...
    * @return letztes Ergebnis von condition */
  def waitFromNowFor(deadlines: IterableOnce[Deadline])(condition: => Boolean)
    (using sleeper: Sleeper = defaultSleeper)
  : Boolean =
  ...
  private[time] def realTimeIterator(deadlines: IterableOnce[Deadline])
    (using sleeper: Sleeper = defaultSleeper)
  : Iterator[Unit] =

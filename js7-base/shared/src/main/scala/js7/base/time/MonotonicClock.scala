...
trait BlockingSleeper extends MonotonicClock:
  def sleepUntil(until: Deadline): Unit

  def sleep(duration: FiniteDuration): Unit


trait RealtimeClock extends MonotonicClock:
  private implicit val implicitClock: MonotonicClock = this

  def now = Deadline(System.currentTimeMillis().ms)

object RealtimeClock extends RealtimeClock


final class RealtimeBlockingSleeper extends RealtimeClock, BlockingSleeper:
  def sleepUntil(until: Deadline): Unit =
...
class TestMonotonicClock(sinceZero: FiniteDuration = ZeroDuration) extends MonotonicClock:
...
object TestMonotonicClock:
  private val logger = Logger[this.type]


final class TestBlockingSleeper(sinceZero: FiniteDuration = ZeroDuration)

...
extends ControllerApiWithHttp:
  ...
  : IO[Checked[Completed]] =
    updateItems(AddVersion(versionId) +: (
      Stream.fromIterable(signedItems).map(o => AddOrChangeSigned(o.signedString)) ++
        Stream.fromIterable(delete).map(RemoveVersioned.apply)))

  def updateUnsignedSimpleItems(items: Iterable[UnsignedSimpleItem]): IO[Checked[Completed]] =
    updateItems(
      Stream.fromIterable(items) map ItemOperation.AddOrChangeSimple.apply)

  def updateSignedItems(items: Iterable[Signed[SignableItem]], versionId: Option[VersionId] = None)
  : IO[Checked[Completed]] =
    updateItems(
      Stream.fromIterable(versionId).map(AddVersion(_)) ++
        Stream.fromIterable(items).map(o => ItemOperation.AddOrChangeSigned(o.signedString)))

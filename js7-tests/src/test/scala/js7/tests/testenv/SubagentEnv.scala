...
extends TestEnv {
  ...
  def convertibleSubagentResource: Resource[Task, ConvertibleSubagent] =
    ConvertibleSubagent.resource(agentConf)

  //def subagentResource: Resource[Task, Subagent] = {
  //  for {
  //    scheduler <- BareSubagent.threadPoolResource[Task](subagentConf)
  //    subagent <- BareSubagent.resource(subagentConf, scheduler)
  //  } yield subagent
  //}
}

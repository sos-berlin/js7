...
import js7.data.order.OrderEvent.{OrderAdded, OrderAttachable, OrderAttached, OrderCancellationMarked, OrderCancelled, OrderDetached, OrderFailed, OrderProcessed, OrderProcessingStarted, OrderStarted, OrderStdoutWritten, OrderTerminated}
...
with BlockingItemUpdater
{
  ...
  "ResetAgent while an Order is being canceled" in {
    ...
    assert(eventWatch.eventsByKey[OrderEvent](orderId) == Seq(
      ...
      OrderDetached,
      OrderFailed(Position(0), Some(Outcome.Disrupted(AgentResetProblem(agentPath)))),
      OrderCancelled))

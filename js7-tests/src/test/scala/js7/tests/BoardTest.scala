  ...
  extends OurTestSuite, ControllerAgentForScalaTest, BlockingItemUpdater
{
  ...
  "ExpectNotices" - {
    "expect(0, 1), post(1, 2), expect(0, 2), post(0)" in {
      ...
      controller.api.addOrders(
        Stream.fromIterable(expecting01OrderIds).map(FreshOrder(_, expecting01Workflow.path))
      ).await(99.s).orThrow
    ...
    "Two orders expect a notice, then post the notice" in {
      ...
      controller.api.addOrders(
        Stream.fromIterable(expectingOrderIds).map(FreshOrder(_, expecting0Workflow.path))
      ).await(99.s).orThrow
    ...
    "PostNotices command with expecting orders" in {
      ...
      controller.api.addOrders(
        Stream.fromIterable(orderIds).map(FreshOrder(_, expectingAgentWorkflow.path))
      ).await(99.s).orThrow
  ...
  "Delete Board" in {
    ...
    controller.api
      .deleteOrdersWhenTerminated(Stream.fromIterable(

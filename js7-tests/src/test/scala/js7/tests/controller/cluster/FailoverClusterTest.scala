...
final class FailoverClusterTest extends ControllerClusterTester
{
  ...
  private def test(addNonReplicatedEvents: Boolean = false): Unit = {
    ...
    withControllerAndBackup() { (primary, backup, clusterSetting) =>
      ...
      val recoupledEventId = primaryController
        .eventWatch.await[ClusterSwitchedOver](after = failedOverEventId).head.eventId
      ...
      val whenClusterWatchAgrees = backupController.testEventBus
        .when[ClusterWatchAgreedToActivation.type].runToFuture
      val whenClusterWatchDoesNotAgree = backupController.testEventBus
        .when[ClusterWatchDisagreedToActivation.type].runToFuture

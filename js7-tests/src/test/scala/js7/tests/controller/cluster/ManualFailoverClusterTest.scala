...
import js7.data.controller.ControllerCommand.{ClusterAcknowledgeLostNode, ShutDown}
...
final class ManualFailoverClusterTest extends ControllerClusterTester
{
  ...
  "Failover and recouple" in {
    withControllerAndBackup(noClusterWatch = true) { (primary, backup, clusterSetting) =>
      ...
      // TODO How to detect ExplicitClusterNodeLostAckRequiredProblem with Proxy ?
      backupController.testEventBus.when[ClusterWatchDisagreedToActivation.type].await(99.s)
      backupController.executeCommandForTest(ClusterAcknowledgeLostNode(primaryId)).orThrow

...
final class ControllerWebServiceTest extends AnyFreeSpec with BeforeAndAfterAll with ControllerAgentForScalaTest
{
  ...
  "Await AgentReady" in {
    ...
    for (agentRefPath <- agentRefPaths) {
      controller.eventWatch.await[ControllerAgentEvent.AgentReady](predicate = _.key == agentRefPath)
    }
  }
  ...
  "/controller/api/agent" - {
    ...
    testGet("controller/api/agent/FOLDER/AGENT-A?return=AgentRef",
      RawHeader("X-JS7-Session", sessionToken) :: Nil,
      json"""{
        "path": "/FOLDER/AGENT-A",
        "versionId": "INITIAL",
    ...
    testGet("controller/api/agent/%2FFOLDER%2FAGENT-A?return=AgentRef",
      RawHeader("X-JS7-Session", sessionToken) :: Nil,
      json"""{
        "path": "/FOLDER/AGENT-A",
        "versionId": "INITIAL",
  ...
  "/controller/api/agent-proxy" - {
    ...
    "/controller/api/agent-proxy/FOLDER%2F/AGENT-A/NOT-FOUND returns 404" in {
      val headers = RawHeader("X-JS7-Session", sessionToken) :: Nil
      assert(
    ...
    "/controller/api/agent-proxy/FOLDER%2F/AGENT-A/timer" in {
      val headers = RawHeader("X-JS7-Session", sessionToken) :: Nil
      assert(
        intercept[HttpException] {
  ...
  "/controller/api/graphql" - {
    "Syntax error" in {
      ...
      val headers = RawHeader("X-JS7-Session", sessionToken) :: Accept(`application/json`) :: Nil
      val response = httpClient.post_[Json](Uri(s"$uri/controller/api/graphql"), queryJson, headers) await 99.s
      assert(response.status.intValue == 400/*BadRequest*/)
    ...
    def postGraphql(graphql: Json): Json = {
      val headers = RawHeader("X-JS7-Session", sessionToken) :: Nil
      httpClient.postWithHeaders[Json, Json](Uri(s"$uri/controller/api/graphql"), graphql, headers).await(99.s)
    }
  }

...
final class IfReturnCodeTest extends FreeSpec {
  ...
  "test" in {
    autoClosing(new DirectoryProvider(List(TestAgentPath))) { directoryProvider ⇒
      ...
      for (a ← directoryProvider.agents) a.job(JobPath("/JOB")).xml = <job><script language="shell">:</script></job>
      for (a ← directoryProvider.agents) a.job(JobPath("/JOB-RC")).xml =
        <job>
...
object IfReturnCodeTest {
  ...
  private val ExpectedEvents = Map(
    ReturnCode(0) → Vector(
      OrderAdded(TestNamedWorkflow.path, Order.StartNow, Payload(Map("RETURN_CODE" → "0"))),
      ...
      OrderFinished),
    ReturnCode(1) → Vector(
      OrderAdded(TestNamedWorkflow.path, Order.StartNow, Payload(Map("RETURN_CODE" → "1"))),
      OrderTransferredToAgent(TestAgentPath),
      OrderProcessingStarted,
      ...
      OrderFinished),
    ReturnCode(2) →  Vector(
      OrderAdded(TestNamedWorkflow.path, Order.StartNow, Payload(Map("RETURN_CODE" → "2"))),
      OrderTransferredToAgent(TestAgentPath),
      OrderProcessingStarted,
  ...
  private def newOrder(orderId: OrderId, returnCode: ReturnCode) =
    Order(orderId, TestNamedWorkflow.path, Order.StartNow, payload = Payload(Map("RETURN_CODE" → returnCode.number.toString)))
}

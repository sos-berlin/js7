...
lazy val jobscheduler = (project in file("."))
  .aggregate(
    install,  // Empty project for .tgz and .zip with complete installation
    agent,
    base,
    common,
    shared,
...
lazy val install = project
  ...
  .settings(
    ...
    //Nexus broken pipe: publishMavenStyle := false,  // Nexus seem to require a pom.xml (otherwise: broken pipe). The jar is for production, not for development, so the pom.xml is of no use.
